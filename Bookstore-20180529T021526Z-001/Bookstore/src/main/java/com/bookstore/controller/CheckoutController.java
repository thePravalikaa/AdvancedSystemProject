...
public class CheckoutController {
		...
		for (CartItem cartItem : cartItemList) {
			...
			if (book.getInStockNumber() <= cartItem.getQty()) {
				BigDecimal bigDecimal = new BigDecimal(cartItem.getBook().getOurPrice())
						.multiply(new BigDecimal(book.getInStockNumber()));
	...
	public String getCartInfo(@ModelAttribute("id") String id, Model model) {

		
		/*List<CartItem> cartItemList = cartItemService
				.findByShoppingCart(shoppingCartService.findById(Long.parseLong(id)));

		
		 * // Selected more than available for (CartItem cartItem : cartItemList) { if
		 * (cartItem.getBook().getInStockNumber() < cartItem.getQty()) {
		 * model.addAttribute("notEnoughStock", true);
		 * cartItem.setOutofStockOrdered(cartItem.getQty()-cartItem.getBook().
		 * getInStockNumber()); cartItemService.save(cartItem); // return
